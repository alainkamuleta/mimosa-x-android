...
private fun SovereignEnrollmentHeader(state: SovereignEnrollmentState) {
    val title = when (state.step) {
        EnrollmentStep.Scanning -> "Enrôlement Souverain"
        EnrollmentStep.Confirming -> "Confirmation d'Organisation"
        EnrollmentStep.Securing -> "Sécurisation de l'Appareil"
        EnrollmentStep.Loading -> "Configuration..."
    }
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
        Spacer(modifier = Modifier.height(24.dp))
        Icon(
            imageVector = if (state.step == EnrollmentStep.Confirming) Icons.Default.VerifiedUser else Icons.Default.QrCodeScanner,
...
@Composable
private fun SovereignConfirmationContent(data: io.element.android.libraries.matrix.api.auth.SovereignEnrollmentData) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(24.dp),
    ...
    Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp)) {
        if (state.step == EnrollmentStep.Confirming) {
            ...
            Button(
                ...
                onClick = { state.eventSink(SovereignEnrollmentEvents.OnBiometricAuthenticated) },
                modifier = Modifier.fillMaxWidth()
            )
        } else {
            TextButton(
